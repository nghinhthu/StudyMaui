...
namespace ThisoApp.Services
{
    public class CustomerService
    {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (!string.IsNullOrEmpty(content))
                    {
                        ...
                        {
                            string profileContent = JsonSerializer.Serialize(getProfileResult.Content, new JsonSerializerOptions()
                            {
                                PropertyNameCaseInsensitive = true,
                                PropertyNamingPolicy = JsonNamingPolicy.CamelCase
                            });
                            IThisoWebCustomerProfileData userInfo = DeserializeExtensions.Deserialize<IThisoWebCustomerProfileData>(profileContent);
                            if (userInfo != null)
                            {
                                result.OK = true;
        ...
        public async Task<BODataProcessResult> UpdateProfile(UpdateProfileRequest request)
        {
            BODataProcessResult result = new BODataProcessResult();
            try
            {
